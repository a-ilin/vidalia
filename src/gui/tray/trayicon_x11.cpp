...
void TrayIcon::TrayIconPrivate::initWM(WId icon)
{
   ...
   classhint.res_name  = (char*)"vidalia";
...
{
   QPainter p(this);
   p.drawPixmap((width() - pix.width())/2, (height() - pix.height())/2, size, size, pix);
}
...
public:
      ...
      : TrayIconPrivate(object, 44)
   {
      // set the class hint
      XClassHint classhint;
      classhint.res_name  = (char*)"vidalia-wharf";
      classhint.res_class = (char*)"Vidalia";
   ...
   void setPixmap(const QPixmap &_pm)
   {
      ...
      i = i.scaledToHeight(i.height() * 2);
      i = i.scaledToWidth(i.width() * 2);
